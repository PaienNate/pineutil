...
import (
	...
	"github.com/PaienNate/pineutil/ftlserver/signals"
)
...
//nolint:gochecknoinits // init function is necessary for initializing global state for endless server
func init() {
	runningServerReg = sync.RWMutex{}
...
const (
	// HTTP_ONLY 仅HTTP服务
	HTTP_ONLY ServerMode = iota
	// HTTPS_WITH_REDIRECT HTTPS服务，HTTP请求重定向到HTTPS
	HTTPS_WITH_REDIRECT
	// HTTPS_AND_HTTP_BOTH 同时提供HTTP和HTTPS服务，不重定向
	HTTPS_AND_HTTP_BOTH
)
...
*/
func NewServer(addr string, handler http.Handler, mode ServerMode) (srv *endlessServer) {
	...
	case HTTPS_WITH_REDIRECT:
		// HTTPS服务，HTTP重定向到HTTPS
		srv.Server.Handler = &hahosp.HandlerSelector{
			HTTPS: handler,
			// HTTP处理器为nil，会自动重定向到HTTPS
		}
	case HTTPS_AND_HTTP_BOTH:
		// 同时提供HTTP和HTTPS服务
		srv.Server.Handler = &hahosp.HandlerSelector{
			HTTPS: handler,
			HTTP:  handler,
...
func ListenAndServe(addr string, handler http.Handler) error {
	server := NewServer(addr, handler, HTTP_ONLY)
...
func ListenAndServeTLS(addr string, certFile string, keyFile string, handler http.Handler) error {
	server := NewServer(addr, handler, HTTPS_WITH_REDIRECT)
	return server.ListenAndServeTLS(certFile, keyFile)
}

/*
ListenAndServeBoth serves both HTTP and HTTPS on the same port without redirecting.
HTTP and HTTPS requests are both handled by the same handler.
*/
func ListenAndServeBoth(addr string, certFile string, keyFile string, handler http.Handler) error {
	server := NewServer(addr, handler, HTTPS_AND_HTTP_BOTH)
	return server.ListenAndServeTLS(certFile, keyFile)
}
...
func (srv *endlessServer) ListenAndServeTLS(certFile, keyFile string) (err error) {
	...
	config, err := GetFakeHttpTLSConfig(certFile, keyFile, srv.TLSConfig)
	if err != nil {
		return err
	}
	...
	// 使用hahosp.VirtualListener来处理HTTP和HTTPS连接
	virtualListener := hahosp.NewVisualListener(srv.tlsInnerListener, config, &srv.Server)
	srv.EndlessListener = virtualListener
...
func (srv *endlessServer) fork() (err error) {
	...
	for _, srvPtr := range runningServers {
		...
		switch listener := srvPtr.EndlessListener.(type) {
		case *endlessListener:
			// normal listener
			files[socketPtrOffsetMap[srvPtr.Server.Addr]] = listener.File()
		default:
			// tls listener
	...
	path := filepath.Join(filepath.Dir(exePath), filepath.Base(exePath))
...
/*
CloseUnusedListeners Close unused listeners, used in forked new process like that:
